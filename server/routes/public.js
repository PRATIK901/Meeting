...
import { db, rows, row, today, uuid, nowIso } from '../db.js';
...
publicRouter.get('/meetings', (_req, res) => {
  res.json(
    rows(
      db.prepare(
...
publicRouter.get('/meetings/:ref', (req, res) => {
  const meeting = resolveMeeting(req.params.ref);
  if (!meeting) {
 ...
 */
publicRouter.get('/meetings/:ref/roster', (req, res) => {
  const meeting = resolveMeeting(req.params.ref);
  if (!meeting) {
    res.status(404).json({ error: 'That meeting no longer exists or is no longer active.' });
    return;
  ...
  res.json(
    rows(
      ...
      day,
      meeting.id,
    ).map((r) => ({ ...r, already_attended: r.already_attended === 1 })),
  );
});
 ...
 * one statement is what enforces every rule the Postgres trigger used to:
 *
 ...
 */
publicRouter.post('/attendance', (req, res) => {
  const { meetingRef, personId } = req.body ?? {};
  const meeting = resolveMeeting(meetingRef);
  if (!meeting) {
    res.status(404).json({ error: 'That meeting no longer exists or is no longer active.' });
    return;
  }
  ...
  try {
    changes = db
      ...
      .run(id, meeting.id, day, at, meeting.id, personId).changes;
  } catch (error) {
